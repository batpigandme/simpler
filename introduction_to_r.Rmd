...
***R can do pretty much everything*.** It's primarily a statistical language, but it's not *just* a statistical language. You can do data analysis in it, sure. You can also visualise the results of that analysis in a thousand different ways. Or write reports based on it that turn out as LaTeX, PDFs, HTML, Markdown. or build dynamic, web-hosted visualisations and dashboards and interactive platforms to display your work. 
...
You should now have RStudio; that's going to be your primary interface for writing R in. Let's briefly step through the important bits of the program that you should be paying attention to.
...
After that will be an explanation of that chunk of code, and what each bit is doing and how everything in it works. There are also likely to be lines starting with `#`: these are comments, which aren't treated as code and run, but instead let you document what the code is doing within your R program.
