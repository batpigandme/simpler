...
The difference is in how you get access, or alter, objects inside the list. If the list has names, you can use the `$` character we use with data frame contents. If it doesn't, you have to use indices - getting the Nth element of the object by pointing to `[[N]]`. Taking the list examples we created above:
